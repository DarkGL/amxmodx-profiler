...
CPluginMngr::CPlugin* CPluginMngr::loadPlugin(const char* path, const char* name, char* error, int debug, int profiling)
{	
	CPlugin** a = &head;
	...
	*a = new CPlugin(pCounter++, path, name, error, debug, profiling);
...
int CPluginMngr::loadPluginsFromFile(const char* filename, bool warn)
{
	...
	while (!feof(fp)) 
	{
		...
		if ((int)CVAR_GET_FLOAT("amx_profile") == 0)
		{
			profileFlag = 0;
		}
		CPlugin* plugin = loadPlugin(pluginsDir, pluginName, error, debugFlag, profileFlag);
...
CPluginMngr::CPlugin::CPlugin(int i, const char* p, const char* n, char* e, int d, int profiling) : name(n), title(n)
{
	if (profiling)
	{
		if ((int)CVAR_GET_FLOAT("amx_debug") >= 2 ||
			d != 0)
		{
			// We do not profile debug plugins
			profiling = 0;
		}
	}
	else
	{
		if ((int)CVAR_GET_FLOAT("amx_profile") >= 2 &&
			d == 0)
		{
			profiling = 1;
		}
	}
	this->m_Profiling = (profiling ? true : false); // yes, this looks stupid, MSVC throws a warning otherwise
	...
	int err = load_amxscript_plugin(&amx, &code, path, e, d, this);
