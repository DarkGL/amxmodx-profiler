...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;                                                                             ;
; void   asm_runJIT( AMX_HEADER *amxh, JumpAddressArray *jumps, void *dest, AMX* amx )  ;
;                                eax                     edx          ebx     ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
_asm_runJIT:
		...
		mov		eax,[esp+32]			; get AMX pointer
		mov		[jit_amxptr],eax		; store AMX pointer
...
;nop;
		...
		pusha
		push 	ebx
		push 	dword j_call_prof
		push 	dword [jit_amxptr]
		call 	[profiler_reloc]
		add 	esp, 12
		
		xor		ebx,ebx
		cmp		eax,ebx
		je		op_call_noprofile
		popa
        RELOC   j_call_e8-j_call_prof+1
        GO_ON   j_call_prof, op_call_noprofile, 8

		j_call_prof:
		pusha
		push 	dword 12345678h ; dummy
		...
		add 	esp, 8
		popa
	CHECKCODESIZE j_call_prof

op_call_noprofile:
		popa		; we're returning from the reloc call, make sure we restore registers!
        RELOC   j_call_e82-j_call_noprof+1
        GO_ON   j_call_noprof, OP_CALL_I, 8
...
jit_amxptr		DD	0	; Saved pointer to the actual AMX (not AMX_HEADER)
