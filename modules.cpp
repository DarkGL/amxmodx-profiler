...
{
	return load_amxscript_plugin(amx, program, filename, error, debug, NULL);
}
int load_amxscript_plugin(AMX *amx, void **program, const char *filename, char error[64], int debug, CPluginMngr::CPlugin* pluginptr)
{
	*error = 0;
	size_t bufSize;
	*program = (void *)g_plugins.ReadIntoOrFromCache(filename, bufSize);
	...
	amx->userdata[UD_FINDPLUGIN] = pluginptr;

	// If this plugin is profiling, then we need to load the debug information
	// When looking through the debug information, we look for symbols that point to functions, and note their names and addresses
	if (pluginptr && pluginptr->isProfiling() && 
		hdr->file_version >= CUR_FILE_VERSION &&
		(hdr->flags & AMX_FLAG_DEBUG) != 0)
	{
		...
		{
			for (int16_t i = 0; i < pDbg->hdr->symbols; i++)
			{
				AMX_DBG_SYMBOL* sym = pDbg->symboltbl[i];

				if (sym->ident == iFUNCTN)
				{
					pluginptr->getProfile().InsertFunctionSymbol(sym->name, sym->address);
				}
			}
			dbg_FreeInfo(pDbg);
			delete pDbg;
...
int set_amxnatives(AMX* amx, char error[128])
{
	...
	amx_Register(amx, profile_Natives, -1);
