...
cvar_t init_amxmodx_profile = {"amx_profile", "1", FCVAR_SPONLY};
...
int	C_Spawn(edict_t *pent)
{
	...
	for (unsigned int i = 0; i < g_ProfileList.size(); i++)
	{
		delete g_ProfileList[i];
	}
	g_ProfileList.clear();
	...
	snprintf(buffer, sizeof(buffer) - 1, "%s + profiler", SVN_VERSION_STRING);
	CVAR_SET_STRING(init_amxmodx_version.name, buffer);
...
C_DLLEXPORT	int	Meta_Attach(PLUG_LOADTIME now, META_FUNCTIONS *pFunctionTable, meta_globals_t *pMGlobals, gamedll_funcs_t *pGamedllFuncs)
{
	...
	CVAR_REGISTER(&init_amxmodx_profile);
